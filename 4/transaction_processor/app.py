...
async def index_page():
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT account_id, current_balance FROM balance order by updated_at desc limit 100")
        result = cursor.fetchall()
        if result:
            res = []
            for row in result:
                res.append({"account_id": row[0], "current_balance": row[1]})
            return {
                "accounts": res
            }
        else:
...
async def create_transaction(transaction: Transaction):
    try:
        ...
        cursor.execute('SELECT id, balance, amount, transaction_type, created_at FROM transactions WHERE external_id = %s;', (transaction.external_id,))
        result = cursor.fetchone()
        if result:
        ...
        ''', (transaction_id, transaction.external_id, transaction.account_id, transaction.amount, balance, transaction.transaction_type))
